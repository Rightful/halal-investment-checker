<template>
  <v-container class="stock-checker">
    <v-alert
      v-if="error"
      type="error"
      class="mb-4"
    ...
    <v-card class="pa-4">
      <h2 class="text-h5 mb-4">Stock Shariah Compliance Checker</h2>
      
      <v-form @submit.prevent="checkCompliance" v-model="isValid">
        <v-text-field
          v-model="ticker"
          label="Enter stock ticker (e.g. AAPL)"
          :rules="[(v: any) => !!v || 'Stock ticker is required']"
          class="mb-4"
          clearable
...
export default defineComponent({
  ...
  setup() {
    ...
    const getScoreDescription = (score: number): string => {
      if (score >= 70) return 'This investment appears to be Shariah-compliant';
      if (score >= 50) return 'This investment requires further analysis';
