<template>
  <v-container class="etf-checker">
    ...
    <v-card class="pa-4">
      <h2 class="text-h5 mb-4">ETF Shariah Compliance Checker</h2>
      
      <v-form @submit.prevent="checkCompliance" v-model="isValid">
        <v-text-field
          v-model="etfSymbol"
          label="Enter ETF Symbol (e.g. SPUS)"
          :rules="[(v: any) => !!v || 'ETF symbol is required']"
          class="mb-4"
          clearable
          placeholder="Enter ETF symbol"
          hint="Enter a valid ETF symbol (e.g. SPUS, HLAL, ISDU)"
          persistent-hint
  ...
  setup() {
    ...
    const getCriteriaDescription = (key: string): string => {
      const descriptions: Record<string, string> = {
        sectorCompliance: 'ETF holdings are in permissible sectors',
        debtCompliance: 'Average debt ratio of holdings meets requirements',
        activitiesCompliance: 'Holdings have no significant non-compliant activities'
      }
      return descriptions[key] || ''
    }

    const getScoreDescription = (score: number): string => {
      if (score >= 70) return 'This ETF appears to be Shariah-compliant'
      if (score >= 50) return 'This ETF requires further analysis'
      return 'This ETF may not be Shariah-compliant'
    }

    const checkCompliance = async () => {
      if (!etfSymbol.value) return

      loading.value = true
      error.value = null

      try {
        result.value = await api.checkInvestment(etfSymbol.value)
      } catch (err) {
        error.value = err instanceof Error ? err.message : 'An error occurred'
        console.error('Error checking ETF compliance:', err)
      } finally {
        loading.value = false
      }
    }
