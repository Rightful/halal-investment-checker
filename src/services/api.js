...
export const api = {
    async checkInvestment(ticker) {
        try {
            ...
            const { data: overviewData } = await axios.get(BASE_URL, {
                params: {
                    function: 'OVERVIEW',
            ...
            const { data: bsData } = await axios.get(BASE_URL, {
                params: {
...
function calculateShariahScore(debtRatio, debtToEquity, issues) {
    let score = 100;
...
function generateRecommendations(product, metrics, debtRatio, debtToEquity) {
    const recommendations = [];
    if (debtRatio > 0.33) {
        recommendations.push(`High debt ratio (${(debtRatio * 100).toFixed(2)}%) exceeds the 33% threshold. Consider alternatives with lower leverage.`);
    }
    if (debtToEquity > 1.0) {
        recommendations.push(`Debt-to-equity ratio (${debtToEquity.toFixed(2)}) is high. Lower financial leverage may enhance compliance.`);
    }
    if (product.forbiddenActivities.length > 0) {
        recommendations.push(`Compliance issues detected: ${product.forbiddenActivities.join('; ')}.`);
    }
    // Optionally, add notes based on other financial metrics
    if (metrics.priceToBook > 30) {
        recommendations.push(`High price-to-book ratio (${metrics.priceToBook.toFixed(2)}) observed; monitor for potential leverage concerns.`);
    }
    if (metrics.evToRevenue > 10) {
        recommendations.push(`EV-to-Revenue ratio of ${metrics.evToRevenue.toFixed(2)} may indicate elevated debt levels.`);
    }
    if (metrics.dividendYield > 0) {
        recommendations.push(`Dividend Yield: ${metrics.dividendYield.toFixed(2)}% (for your information).`);
    }
